 ...
 */
async function main() {
  let [chainId, scriptName, skipJsonFlag, rpcUrl, force, broadcastDir] = validateAndExtractInputs();
  if (rpcUrl === undefined) {
    console.warn("\u{26A0} No rpc url provided, skipping version fetching");
  }
  let json;
  if (!skipJsonFlag) json = await extractAndSaveJson(scriptName, chainId, rpcUrl, force, broadcastDir);
  else {
